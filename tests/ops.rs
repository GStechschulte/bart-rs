...
#[test]
fn test_constant_response_sample_leaf_value() {
    let sampling_ops = TreeSamplingOps {
        normal: Normal::new(0.0, 1.0).unwrap(),
        alpha_vec: vec![],
        splitting_probs: vec![],
    ...
    let result = sampling_ops.sample_leaf_value(&mu, &obs, m, &leaf_sd, &shape, &response_constant);

    assert!((result - 1.0).abs() < 1e-12);
}

#[test]
fn test_linear_response_sample_leaf_value_len_2() {
    let sampling_ops = TreeSamplingOps {
        normal: Normal::new(0.0, 1.0).unwrap(),
        alpha_vec: vec![],
        splitting_probs: vec![],
    ...
    let result = sampling_ops.sample_leaf_value(&mu, &obs, m, &leaf_sd, &shape, &response_linear);
